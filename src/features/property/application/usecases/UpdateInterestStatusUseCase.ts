...
export class UpdateInterestStatusUseCase {
  ...
  async execute(data: {
    ...
    if (property.ownerId !== data.ownerId) {
      throw new AppError(
        "Solo el propietario puede aceptar o rechazar intereses",
        403,
      );
    }
    ...
    if (interest.status === data.status) {
      throw new AppError(
        `El interés ya está ${data.status === "aceptado" ? "aceptado" : "rechazado"}`,
        400,
      );
    }
      ...
      if (data.status === "aceptado") {
        ...
        try {
          const socketPayload = {
            ...
            content: `${ownerName} aceptó tu interés en "${propertyTitle}"`,
            metadata: {
        ...
        try {
          const socketPayload = {
            ...
            content: `${ownerName} rechazó tu interés en "${propertyTitle}"`,
            metadata: {
              propertyId: property.id,
              interestId: interest.id,
            },
